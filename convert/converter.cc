...
namespace ufg
{
  ...
  namespace
  {
    ...
    // Converts the filename to a valid absolute SdfPath by removing any
    // extension, converting to a valid identifier, then prepending with '/'. See
    // usd/pxr/base/tf/stringUtils.h:TfIsValidIdentifier for more details.
    ...
    void SetTranslationSkinKeys(const UsdSkelAnimation &skel_anim,
                                const NodeInfo *const *joint_infos,
                                size_t ujoint_count,
                                const VtArray<GfVec3f> &rest_points, float time_min,
                                float time_max)
    {
    ...
    void SetRotationSkinKeys(const UsdSkelAnimation &skel_anim,
                             const NodeInfo *const *joint_infos,
                             size_t ujoint_count,
                             const VtArray<GfQuatf> &rest_points, float time_min,
                             float time_max,
    ...
    {
      return GfVec3f(GetNormalizedScale(scale[0]), GetNormalizedScale(scale[1]),
                     GetNormalizedScale(scale[2]));
    }

    GfVec3f SetScaleSkinKeys(const UsdSkelAnimation &skel_anim,
                             const NodeInfo *const *joint_infos,
                             size_t ujoint_count,
                             const std::vector<GfVec3f> &rest_points,
                             float time_min, float time_max, bool normalize,
                          ...
                          const SdfLayerRefPtr &layer, Logger *logger)
  {
    ...
    {
      Log<UFG_ERROR_ASSERT>(logger, "", e.GetFile(), e.GetLine(),
                            e.GetExpression());
      return false;
    }
  }
  ...
  GfVec3f Converter::CreateSkelAnim(const SdfPath &path,
                                    const SkinInfo &skin_info,
                                    ...
                                    std::vector<GfVec3f> *out_frame0_scales)
  {
    ...
    const GfVec3f root_scale =
        SetScaleSkinKeys(skel_anim, joint_infos.data(), ujoint_count, rest_scales,
                         time_min, time_max, cc_.settings.normalize_skin_scale,
                         ujoint_roots, out_frame0_scales);

    return root_scale;
  }

  void Converter::CreateMesh(size_t mesh_index, const SdfPath &parent_path,
                             bool reverse_winding,
                             const SkinnedMeshContext *skinned_mesh_context)
  {
    ...
    for (size_t prim_index = 0; prim_index != prim_count; ++prim_index)
    {
      ...
      // it, which doesn't support instancing (a mesh being replicated at
      ...
      if (material)
      {
        for (const auto &uvset_kv : material_binding->uvsets)
        {
          ...
          const UsdGeomPrimvar uvs_primvar =
              pxrInternal_v0_24__pxrReserved__::UsdGeomPrimvarsAPI(usd_mesh)
                  .CreatePrimvar(uvset_tok, SdfValueTypeNames->TexCoord2fArray,
      ...
      if (color_scalar_count > 0)
      {
        ...
        if (!AllNearlyEqual(color_scalars, color_scalar_count, 1.0f, kColorTol))
        {
          ...
          // targeting the iOS viewer which doesn't support it. And while
          // Usdview supports it, it's not compatible with texturing because you
          // must replace the color input normally used for texturing (and the
          // color multiplier field does not work).
                                      ...
                                      bool reverse_winding)
  {
    ...
    for (const Gltf::Id node_id : node_ids)
    {
      ...
      // Create SkelRoot, Skeleton, and SkelAnimation the first time this skin
      // is referenced.
      ...
      const SkinnedMeshContext skinned_mesh_context = {
          skel.skeleton_path, skel.anim_path, joint_map.data(), joint_map.size(),
                                      ...
                                      const GfMatrix4d &parent_world_mat)
  {
    ...
    if (!info.is_animated)
    {
      ...
      // can treat it as static. Note for skinned meshes, animation data is
      // stored separately under SkelRoot.
    ...
    if (node.camera != Gltf::Id::kNull)
    {
      ...
      LogOnce<UFG_WARN_CAMERAS_UNSUPPORTED>(&cc_.once_logger,
                                            " Node(s): ", src_node_name.c_str());
    }
    ...
    // From my reading of usdSkel/utils.cpp, vector-matrix multiplication has
    // the vector on the left, so the convention appears to be that matrix
    // multiplication is ordered local*world. Either way, it's currently
    // arbitrary for our purposes because we're only using the world matrix to
    // determine when we need to reverse winding for inverse-scale.
    ...
    // in the hierarchy, in which case this duplicates mesh data. I'm not sure
    // if there's a way to instance meshes in USD, though.
  ...
  void Converter::CreateNodes(const std::vector<Gltf::Id> &root_nodes)
  {
    ...
    for (size_t pass = 0; pass != kPassCount; ++pass)
    {
      ...
      // Skinned meshes may have been reanchored to the root (which doesn't have
      // a source node).
                              ...
                              const SdfLayerRefPtr &layer, Logger *logger)
  {
    ...
    const std::vector<Gltf::Id> scene_nodes = GetNodesUnderRoots(
        *cc_.gltf, root_nodes, cc_.settings.remove_node_prefixes);
    ...
    // hierarchy, we will normally treat those as rigidly animated. But since
    // the iOS viewer doesn't skin normals, that will cause normals to only be
    // animated by a partial subset of transforms, which usually looks worse
    // than not animating them at all. This work-around forces all animated
    // joints to be skinned, even if they are not referenced by the mesh. This
    // is likely to make the animation larger, and won't fix all cases.
