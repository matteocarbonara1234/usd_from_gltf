...
namespace
{
  ...
  bool HasExtension(const std::vector<Gltf::ExtensionId> &extensions,
                    Gltf::ExtensionId key)
  {
  ...
  private:
    ...
    void ValidateIdFieldArray(const char *name, size_t size, const Gltf::Id *ids,
                              size_t count)
    {
    ...
    void ValidateInsideBufferView(Gltf::Id view_id,
                                  Accessor::ComponentType component_type,
                                  Accessor::Type type, size_t count,
                                  size_t offset)
    {
    ...
    void ValidateRangedField(const char *name, const Value &value, Limit lower,
                             Limit upper)
    {
      const GltfPathStack::Sentry path_sentry(&path_stack_, name);
                  ...
                  const Animation::Sampler *sampler)
    {
      ...
      if (input_accessor && output_accessor)
      {
        ...
        {
          Log<GLTF_ERROR_NON_MULTIPLE_SIZE>(output_scalar_count,
      ...
      if (v.path == Animation::Channel::Target::kPathWeights && sampler)
      {
        ...
        if (output_accessor)
        {
          auto it = std::find(gltf_->extensionsUsed.begin(), gltf_->extensionsUsed.end(), Gltf::kExtensionQuant);
          if (it == gltf_->extensionsUsed.end())
          {
    ...
    void Validate(const BufferView &v)
    {
      ...
      if (buffer)
      {
        ...
        {
          Log<GLTF_ERROR_VIEW_OFFSET_GT_BUFFER_SIZE>(v.byteOffset,
                                                     buffer->byteLength);
        }
        else
        {
                                     ...
                                     const Mesh::AttributeSet &other_attrs)
    {
      for (const Mesh::Attribute &attr : attrs)
      {
        ...
        if (is_joints || is_weights)
        {
          ...
          {
            Log<GLTF_ERROR_SKIN_JOINT_WEIGHT_MISMATCH>(attr.ToString().c_str(),
                                                       key.ToString().c_str());
          }
        }
      }
    ...
    void Validate(const Mesh::Primitive &v, size_t morph_target_count)
    {
      ...
      {
        Log<GLTF_ERROR_BAD_PRIMITIVE_INDEX_COUNT>(Gltf::GetEnumName(v.mode),
                                                  index_count);
      }
      ...
      {
        Log<GLTF_ERROR_MORPH_TARGET_MISMATCH>(v.targets.size(),
                                              morph_target_count);
      }
      ...
      if (material)
      {
        CheckTexcoord("baseColorTexture", material->pbr.baseColorTexture, attrs,
                      draco_attrs);
        CheckTexcoord("metallicRoughnessTexture",
                      material->pbr.metallicRoughnessTexture, attrs, draco_attrs);
        if (material->pbr.specGloss)
        {
          CheckTexcoord("diffuseTexture", material->pbr.specGloss->diffuseTexture,
                        attrs, draco_attrs);
          CheckTexcoord("specularGlossinessTexture",
                        material->pbr.specGloss->specularGlossinessTexture, attrs,
                        draco_attrs);
        }
        CheckTexcoord("normalTexture", material->normalTexture, attrs,
                      draco_attrs);
        CheckTexcoord("occlusionTexture", material->occlusionTexture, attrs,
                      draco_attrs);
        CheckTexcoord("emissiveTexture", material->emissiveTexture, attrs,
                      draco_attrs);
      }
    }
    ...
    void Validate(const Skin &v)
    {
      ...
      if (ibm_accessor)
      {
        ...
        {
          Log<GLTF_ERROR_SKIN_JOINT_BIND_MISMATCH>(v.joints.size(),
                                                   ibm_accessor->count);
        }
      }
    }
  ...
  const char *GetBufferOrImageRef(const std::vector<Element> &elements,
                                  size_t index)
  {
  ...
  bool VerifyResourcesExist(GltfStream *gltf_stream, const Gltf &gltf,
                            GltfLogger *logger)
  {
...
bool GltfLoadAndValidate(GltfStream *gltf_stream, const char *name,
                         const GltfLoadSettings &settings, Gltf *out_gltf,
                         GltfLogger *logger)
{
